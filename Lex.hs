...
scan1M :: (Applicative m, MonadExcept e m, MonadState LexerState m) => e -> m Token
scan1M e = ML.gets lexInput >>= \
           case [] -> return EOF
                xs -> case findLongestPrefix (withMatched $ reSpace *> reLexeme <* reSpace) xs
                      of Nothing -> throw e
